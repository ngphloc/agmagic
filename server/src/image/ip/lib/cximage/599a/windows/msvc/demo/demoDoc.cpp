...
BOOL CDemoDoc::OnNewDocument()
{
	...
	image = NULL;
	m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
...
BOOL CDemoDoc::OnOpenDocument(LPCTSTR lpszPathName) 
{
	...
	Stopwatch(0);
	image=newMyImage();	image->Load(filename,type);
	Stopwatch(1);
		...
		if (AfxMessageBox(s,MB_OKCANCEL)==IDOK){
			...
			for(int i=1;i<image->GetNumFrames();i++){
				...
				if (NewDoc)	{
					...
					NewDoc->m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
				}
			}
		}
	}
...
BOOL CDemoDoc::DoSave(LPCTSTR pszPathName, BOOL bReplace /*=TRUE*/)
{
	...
	else if (!pTheApp->GetWritableType(image->GetType()))	bSaveAs = TRUE;
	...
	if (bSaveAs){
		...
		if (bReplace && newName.IsEmpty()){
			...
			if (image->GetType()) newName += pTheApp->GetExtFromType(image->GetType()).Mid(1,4);
		}

		int nDocType = image->GetType();
		if (!pTheApp->PromptForFileName(newName, 
...
{
	pCmdUI->Enable(0);
//	pCmdUI->Enable((image != NULL)); //&& pTheApp->GetWritableType(image->GetType()));
}
//////////////////////////////////////////////////////////////////////////////
...
void CDemoDoc::OnTransformEllipse() 
{
	...
	CxImage *image2 = newMyImage(image,false,false,true);
...
void CDemoDoc::OnWindowDuplicate() 
{
	...
	if (NewDoc)	{
		CxImage *newImage = newMyImage(image);
...
void CDemoDoc::SubmitUndo()
{
	...
	if (image->IsValid()){
		imageUndo[m_UndoLevel] = newMyImage();
...
void CDemoDoc::UpdateStatusBar()
{
	...
	if (image){
		...
		CString s,t;
		t = pTheApp->GetDescFromType(image->GetType());
...
void CDemoDoc::OnCximageOptions() 
{
	...
	if (dlg.DoModal()==IDOK){
		...
		pTheApp->m_optJpegQuality = dlg.m_quality;
#endif
	}
}
//////////////////////////////////////////////////////////////////////////////
...
void CDemoDoc::OnCximageSplitrgb() 
{
	...
	CxImage *newr = newMyImage();
	CxImage *newg = newMyImage();
	CxImage *newb = newMyImage();
	...
	if (NewDocr)	{
		...
		NewDocr->m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
	}
	...
	if (NewDocg)	{
		...
		NewDocg->m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
	}
	...
	if (NewDocb)	{
		...
		NewDocb->m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
	}
}
//////////////////////////////////////////////////////////////////////////////
...
void CDemoDoc::OnCximageSplitcmyk() 
{
	...
	CxImage *newc = newMyImage();
	CxImage *newm = newMyImage();
	CxImage *newy = newMyImage();
	CxImage *newk = newMyImage();
	...
	if (NewDock)	{
		...
		NewDock->m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
	}
}
//////////////////////////////////////////////////////////////////////////////
...
void CDemoDoc::OnViewToolsText() 
{
    ...
    memcpy( &(dlg.m_textdata), &(pTheApp->m_text), sizeof(CXTEXTINFO) );
    if (dlg.DoModal()==IDOK)
    {
        //retrieve all data about text
		memcpy( &(pTheApp->m_text), &(dlg.m_textdata), sizeof(CXTEXTINFO) );
		m_tool=3;
	}
#endif
}
...
void CDemoDoc::OnCximageFft() 
{
	...
	if (dlg.DoModal()==IDOK){
		...
		if (srcr) dstr = newMyImage(srcr,true,false,false); else dstr=0;
		if (srci) dsti = newMyImage(srci,true,false,false); else dsti=0;

		if (dstr==0){
			dstr = newMyImage(dsti->GetWidth(),dsti->GetHeight(),8);
			dstr->Clear(0);
			dstr->SetGrayPalette();
		}
		if (dsti==0){
			dsti = newMyImage(dstr->GetWidth(),dstr->GetHeight(),8);
			dsti->Clear(0);
			dsti->SetGrayPalette();
		}
		...
		if (NewDoci)	{
			...
			NewDoci->m_DocID=((CDemoApp*)AfxGetApp())->m_nDocInc++;
		}
		CDemoDoc *NewDocr=(CDemoDoc*)((CDemoApp*)AfxGetApp())->demoTemplate->OpenDocumentFile(NULL); 
		if (NewDocr)	{
			NewDocr->image = dstr;
