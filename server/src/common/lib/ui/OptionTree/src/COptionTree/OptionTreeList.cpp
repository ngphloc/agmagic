...
void COptionTreeList::OnContextMenu(CWnd* pWnd, CPoint point) 
{
	UNREFERENCED_PARAMETER(pWnd);
	ASSERT(m_otOption!=NULL);

	ScreenToClient(&point);
	COptionTreeItem *pItem = m_otOption->FindItem(point);

	if(pItem)	m_otOption->SendNotify(OT_NOTIFY_CONTEXTMENU, pItem);
}
