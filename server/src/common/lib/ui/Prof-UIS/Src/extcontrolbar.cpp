...
CPoint ptHelperDragOffsetDefault(
	...
	ASSERT( hWndOwn != NULL && ::IsWindow(hWndOwn) );
const UINT nCheckTimerID = 4100;
	::SetTimer( hWndOwn, nCheckTimerID, 50, NULL );
	while( m_bDragging && ::IsWindow(hWndOwn) )
	{
		...
		while( ::IsWindow(hWndOwn) && PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE) )
		{
			...
			{
			case WM_TIMER:
				if( UINT(msg.wParam) == nCheckTimerID )
				{
...
//							);
	...
	if(  ::IsWindow(hWndOwn) )
		::KillTimer( hWndOwn, nCheckTimerID );
}
...
bool CExtBarNcAreaButtonAutoHide::OnNcAreaClicked( CPoint point )
{
	...
	if( m_rc.PtInRect(point) )
	{
		...
		CFrameWnd * pFrame = CExtBarNcAreaButton::GetDockingFrame();
		if( pFrame != NULL )
		{
			ASSERT_VALID( pFrame );
			pFrame->DelayRecalcLayout();
