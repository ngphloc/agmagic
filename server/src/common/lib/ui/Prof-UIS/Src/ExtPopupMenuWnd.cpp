...
//#define ID_TIMER_DELAY_HIDE 663
//#define ID_PERIOD_DELAY_HIDE 500
	...
	)
{
	...
	return Paint( dc, m_bMakePhotosOnly );
}
...
CExtPopupMenuWnd::CExtPopupMenuWnd()
	...
	, m_hWndCmdReceiver( m_hWndCmdReceiver )
	, m_nLeftAreaWidth( 0 )
#if (!defined __EXT_MFC_NO_CUSTOMIZE)
	, m_pNode( NULL )
	, m_pDragSrcNode( NULL )
...
int CExtPopupMenuWnd::OnQueryMenuBorderSize() const
{
	ASSERT_VALID( this );
int nMenuBorderSize =
		g_PaintManager->GetMenuBorderSize();
	ASSERT( nMenuBorderSize >= 0 );
	return nMenuBorderSize;
}
...
CExtLocalResourceHelper _LRH;
	...
	if( ! sTip.LoadString(nTranslatedResourceCmdID) )
	{
		sTip = pCmdItem->m_sTipStatus;
		return;
	} // if( ! sTip.LoadString(nTranslatedResourceCmdID) )
	else
	{
		...
		sTip = sBuffer;
	} // else from if( ! sTip.LoadString(nTranslatedResourceCmdID) )
}
...
//void CExtPopupMenuWnd::_DelayHide( bool bDelay )
//{
//	ASSERT_VALID( this );
//	if(		m_hWnd == NULL
//		||	(! ::IsWindow(m_hWnd) )
...
//	m_bDelayedHiding = false;
//	if( m_pWndParentMenu != NULL )
//		m_pWndParentMenu->m_nDelayedHidingItemIndex = IDX_NOTHING;
//	KillTimer( ID_TIMER_DELAY_HIDE );
//	if( bDelay )
//	{
//		m_bDelayedHiding = true;
//		if( m_pWndParentMenu != NULL )
//		{
//			ASSERT( m_pWndParentMenu->m_nCurIndex >= 0 );
//			//ASSERT( m_pWndParentMenu->ItemGetPopup( m_pWndParentMenu->m_nCurIndex ) == this );
//			m_pWndParentMenu->m_nDelayedHidingItemIndex =
//				m_pWndParentMenu->m_nCurIndex;
//		}
//		SetTimer( ID_TIMER_DELAY_HIDE, ID_PERIOD_DELAY_HIDE, NULL );
//		return;
//	} // if( bDelay )
//}
...
{
	ASSERT_VALID( this );
//	if( m_hWnd != NULL && m_bDelayedHiding )
//	{
//		if( m_pWndParentMenu != NULL )
//			m_pWndParentMenu->m_nDelayedHidingItemIndex = IDX_NOTHING;
//		return TRUE;
...
CExtPopupMenuSite & _site = GetSite();
		...
		)
	{
		...
		_mpmd.SendMessage( m_hWndCmdReceiver );
...
MENUITEMDATA & mi = _GetItemRef(nHitTest);
	...
	if( mi.IsPopup() )
	{
		...
		if( mi.IsInplaceEdit() && bInplaceDropDownArea )
			_ItemFocusCancel( FALSE );
		if( m_nDelayedFocusItemIndex < 0 ) // fix 2.24 condition for care about current animation status
			_ItemFocusSet(
				nHitTest,
				TRUE,
				TRUE
			...
			)
		{
				...
				)
			{
				...
				if( bNoEat )
					return false;
				if(		::IsWindow(hWndOwn)
					&&	((::GetClassLong(hWndOwn,GWL_STYLE))&WS_VISIBLE) != 0
					)
					_OnCancelMode();
				return true;
			}
...
//			||
//			_iterLast == m_items_all.GetSize()
//_iterLastVisSep == m_items_all.GetSize()-1
_iterLastVisSep == _iterLast
			)
		{
