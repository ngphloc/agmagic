...
//	AddAnchor( IDC_CHECK_SHOW_BORDERS,			CSize( 0,  0) );
	...
	::CheckDlgButton( 
		...
		m_wndDatePicker.GetDatePickerStyle() & __EDPWS_MULTIPLY_SELECTION
		? BST_CHECKED
		: BST_UNCHECKED		
		);
...
BOOL CPageCalendar::OnCmdMsg(UINT nID, INT nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo) 
{
	...
	if( nCode == CN_COMMAND )
	{
		...
		case IDC_CHECK_ENABLE_MULTIPLE_SELECT:
			IsDlgButtonChecked( nID )
				? m_wndDatePicker.ModifyDatePickerStyle( 0, __EDPWS_MULTIPLY_SELECTION )
				: m_wndDatePicker.ModifyDatePickerStyle( __EDPWS_MULTIPLY_SELECTION, 0 );
			break;
