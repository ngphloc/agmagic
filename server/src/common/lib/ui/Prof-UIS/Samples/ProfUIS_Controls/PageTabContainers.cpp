...
BOOL CPageTabContainers::OnInitDialog() 
{
	...
	m_wndTabPageDialog0.AddAnchor(
		IDC_EDIT_IN_TAB_PAGE_DIALOG,
		__RDA_LT,
		__RDA_RT
		);
	VERIFY(
		m_wndEditInTabPageDlg0.SubclassDlgItem(
		IDC_EDIT_IN_TAB_PAGE_DIALOG,
		&m_wndTabPageDialog0
		)
		&&	m_wndLableInTabPageDlg0.SubclassDlgItem(
		IDC_STATIC_IN_TAB_PAGE_DIALOG,
		&m_wndTabPageDialog0
		)
		);
	...
	VERIFY( m_wndTabPageContainerFlat.PageInsert( &m_wndColorCtrl0, _T("ColorCtrl"), hIconColor ) );
	VERIFY( m_wndTabPageContainerFlat.PageInsert( &m_wndListBox0, _T("ListBox") ) );
	VERIFY( m_wndTabPageContainerFlat.PageInsert( &m_wndTreeCtrl0, _T("TreeCtrl"), hIconTree ) );
	VERIFY( m_wndTabPageContainerFlat.PageInsert( &m_wndEditCtrl0, _T("EditCtrl"), hIconEdit ) );
	...
	m_wndTabPageDialog1.AddAnchor(
		IDC_EDIT_IN_TAB_PAGE_DIALOG,
		__RDA_LT,
		__RDA_RT
		);
	VERIFY(
		m_wndEditInTabPageDlg1.SubclassDlgItem(
		IDC_EDIT_IN_TAB_PAGE_DIALOG,
		&m_wndTabPageDialog1
		)
		&&	m_wndLableInTabPageDlg1.SubclassDlgItem(
		IDC_STATIC_IN_TAB_PAGE_DIALOG,
		&m_wndTabPageDialog1
		)
		);
	...
	VERIFY( m_wndTabPageContainer.PageInsert( &m_wndColorCtrl1, _T("ColorCtrl"), hIconColor ) );
	VERIFY( m_wndTabPageContainer.PageInsert( &m_wndListBox1, _T("ListBox") ) );
	VERIFY( m_wndTabPageContainer.PageInsert( &m_wndTreeCtrl1, _T("TreeCtrl"), hIconTree ) );
	VERIFY( m_wndTabPageContainer.PageInsert( &m_wndEditCtrl1, _T("EditCtrl"), hIconEdit ) );
...
BOOL CPageTabContainers::InitListBoxCtrl(CListBox *pListBox, CExtTabPageContainerWnd *pTabsContainer)
{
	...
	pListBox->AddString(_T("Prof-UIS multiply monitor support:"));
	pListBox->AddString(_T("     built-in"));
	pListBox->AddString(_T(" Automatically linking with ProfUIS224 library"));
