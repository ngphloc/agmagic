    ...
    if ((m_button_down) && (::GetCapture() == m_hWnd) && (::GetCursorPos(&mouse_position))){
		if (::WindowFromPoint(mouse_position) == m_hWnd){
			if ((GetState() & BST_PUSHED) != BST_PUSHED) {
				//TRACE(_T("* Inconsistency up detected! Fixing.\n"));
				SetState(TRUE);
				return;
			}
		} else {
...
void CxTargetButton::SetIcon(UINT nIcon, UINT nIconAlign, UINT nIconDown, UINT nIconHighLight)
{
	...
	hInst = AfxFindResourceHandle(MAKEINTRESOURCE(nIcon),RT_ICON);
	m_Icon = (HICON)::LoadImage(hInst/*AfxGetInstanceHandle()*/,
								MAKEINTRESOURCE(nIcon),
								IMAGE_ICON, 0, 0, 0);
	//Co tim trong DLL
	if(m_Icon==NULL)
	{
		hInst=CAGmagicMicUtil::FindResourceHandleInDynamicDLL(MAKEINTRESOURCE(nIcon),RT_ICON);
		//Lay mac dinh la IDB_AGMAGICIPBITMAP neu nhu hInst=NULL
		if(hInst==NULL)	hInst = AfxFindResourceHandle(MAKEINTRESOURCE(IDB_AGMAGICIPBITMAP),RT_BITMAP);
		m_Icon = (HICON)::LoadImage(hInst/*AfxGetInstanceHandle()*/,
									MAKEINTRESOURCE(nIcon),
									IMAGE_ICON, 0, 0, 0);
	}
	...
	if (m_Icon){							// if success...
		...
		if (nIconDown > 0){	//load down icon
			m_IconDown = (HICON)::LoadImage(hInst/*AfxGetInstanceHandle()*/,
		...
		if (nIconHighLight > 0){	//load highlighted icon
			m_IconHighLight = (HICON)::LoadImage(hInst/*AfxGetInstanceHandle()*/,
